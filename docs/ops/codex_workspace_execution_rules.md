...
- 基线前进时默认复用现有唯一候选工作树，禁止仅因主线 SHA 变化就复制一个新目录。必须使用
  `make workspace.worktree.baseline.update`：未发布分支可选 `UPDATE_WORKTREE_MODE=rebase`，已发布分支
  必须使用 `merge` 保留历史。入口绑定更新前完整 HEAD 与当前精确 `origin/main`，要求工作树干净；
  冲突时自动中止并恢复原 HEAD。更新成功后重新冻结完整指纹并从最早受影响门禁继续。
- 只有受控基线更新无法安全保持候选、且冲突处置已经形成证据时，才允许创建替代工作树。替代候选
  验证通过并固化后，旧工作树必须在同一迭代内通过受控清理入口移除；不得长期并存两个活跃长分支目录。
- 常态资源上限为“主工作树 + 1 个活跃长产品工作树 + 最多 2 个短专题工作树”。短专题合并并核验主线后
  必须立即清理；带未提交内容的工作树一律保留并先定责，禁止按目录名称推断可删除。

...
3. **按风险逐级验证**
   ...
   - **先判影响面，再选验证层级。** 仅文档、测试断言、单个适配器或局部样式改动，默认只执行
     restricted/定向验证；不得因为“页面可能受影响”直接启动候选浏览器矩阵。
   - **浏览器候选只在产品批次冻结后启动。** 候选构建前必须先完成代码层、契约层和范围守卫；
     构建后只覆盖实际受影响页面与视口，不得把历史全矩阵当作每次迭代的默认门禁。
   - **候选启动前必须做短路健康检查。** 在登录或页面等待前验证服务健康、候选 SHA、数据库身份和
     测试凭据均可用；任一前提失败立即停止并归类 `ENV_UNSTABLE`，不得让脚本进入长超时等待。
   - **单次验证必须有明确退出时限。** 页面加载、单个交互和候选启动分别使用受管入口的短超时；
     超时只记录一次失败证据并停止该层，不得重复同一命令消耗等待时间。只有修复了失败归因或
     明确恢复了环境，才允许重试。
   - **失败不自动升级范围。** 定向验证失败时先归因到 CODE/CONTRACT/GUARD/ENV；未证明受影响的
     其他页面、业务流程、角色和主题不得被顺手加入验证。需要扩大范围必须重新声明批次目标。
   - **交付门禁与迭代证据分离。** 纯文档或不影响产品运行时，不因 HEAD 变化机械重跑浏览器；
     PR/合并所需 Quick 证据应在交付 HEAD 冻结后集中补齐，并明确哪些产品候选证据可复用、哪些不可复用。

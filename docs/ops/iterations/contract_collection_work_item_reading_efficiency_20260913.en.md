...
| L4 | Read-only 1088 product check | The static build inside `local.dev.candidate.frontend.up` passed, but the existing 5176 identity guard denied candidate-service startup; the page check has not run and static results are not presented as product acceptance |
...
The specific L4 blocker is `/tmp/sc-local-dev-candidate-frontend.pid`, which remains bound to the deleted `/home/lidefend/workspace/sce-backend-odoo-batch2a-personnel-auth` worktree, old head `23225efbb8bf5bb8b0276af7ec016eb5e5614612`, and a still-running static service on port 5176. This product topic will neither terminate that process manually, assemble a new port, nor add a P4 validation entrypoint. The existing carrier's governance owner must release the stale allocation before this topic's clean candidate can run the read-only 1088 check.

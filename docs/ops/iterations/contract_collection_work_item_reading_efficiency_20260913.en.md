...
| L4 | Focused My Work checks at `ed36ba45…` | PASS; 1088×791 shows two complete collapsed cards, 1440×960 has no large empty block, and 390×844 wraps title, amount, and actions naturally. Keyboard covered disclosure at 1088/1440, touch covered 390; all summaries report zero mutations, zero errors, and zero root-page horizontal overflow |
| L5 preparation | `make ci.delivery.freeze.prepare` | PASS; 1366 inventory entries, seven component-inventory tests, 35 required adapters, and the 1866-line form-split evidence are current; the only generated diff updates the component-takeover input digest |
...
The P0 density-correction candidate `ed36ba45d285f0bce77c2579c260dae6f5443d69` (complete fingerprint `b7c0522328d18e6d98fce8c4735c62f78c0b8872043337b0957ed5b56e133473`, 7427 paths) completed focused validation. The 1088 collapsed sample passed manual product review, while actual 1440 light and 390 light screenshots confirmed stable reading relationships. The existing `exerciseFactDisclosure` opened with Enter and closed with Space at 1088/1440, used touch at 390, and matched the complete identity to the collapsed title; every summary has `pass=true` and `mutationCount=0`. The public `ScButton :focus-visible` rule supplies the focus ring, and Submit for approval was never clicked or triggered. Evidence directories are:
...
One attempt applied a form-only bulk-expansion/field-alignment screenshot option to My Work; the carrier exited before producing evidence. This was classified as a validation-tool applicability error, was not retried unchanged, and did not lead to a P4 tool change. The disclosure conclusion relies only on the existing My Work-specific capability above. The remaining steps are generated-evidence preflight, documentation commit, one Quick run on the final HEAD, and exact-head independent review; contract journeys, save flows, fixtures, and business writes remain not run.

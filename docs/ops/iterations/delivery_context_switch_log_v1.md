...
- Formal release/browser acceptance remains blocked by that earlier static gate. The governed local.dev candidate visual smoke is limited to read-only development evidence for `/`, `/s/workspace.home`, `/my-work` and `/s/my_work.workspace`, with exact alias geometry and 1440/390 overflow checks; result is pending at this checkpoint. No push, PR or merge is authorized.

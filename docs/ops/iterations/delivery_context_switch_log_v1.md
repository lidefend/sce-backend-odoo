...
- Formal Product Layer P1/P0；Layer Target 为 `smart_construction_core` 的 action 605 入口专用原生视图，以及 `smart_core` 对原生表单能力的通用契约收敛。项目看板继续承接投标、合同、工程资料的汇总查看，但入口专用表单根节点明确禁止创建、编辑、删除和复制；默认项目表单、项目信息专用表单、业务对象专用入口、模型权限与删除协议均不改变。

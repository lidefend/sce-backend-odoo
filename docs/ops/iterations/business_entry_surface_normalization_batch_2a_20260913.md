...
- 旅程同时如实记录 3 次辅助 `res.users api.onchange` HTTP 500；根因位于主线既有 onchange
  ...
  原始旅程 runner `92bef311…` 留存了全部失败。独立审查后，当前 runner 进一步要求 HTTP 500、
  错误码/文案、受控阶段、intent/model/record、请求 URL 全部匹配，并与同阶段/同 URL/3 秒内的精确
  console 错误一一配对；发生次数由实际请求自动统计，不固化单次运行观测值。403、不同 500、错误阶段、
  未配对 console 及 pageerror 均由行为反例证明会阻断。本批不宣称修复该历史 onchange 问题，也不因
  工具加固重复业务写入旅程。

...
合同组用户复核通过，迁移台账50→48；专题尚未发布。导航只定位折叠标题，手动展开正常，不登记自动展开。
...
L1 静态与结构测试；L2 结算定向模块测试；L3 受管升级；L4 两入口创建、已有查看及真实可用草稿，缺失态明确登记。合同产品/前端证据按完整指纹和相关文件一致性承接。本组仍不运行 Quick，待结算用户复核后统一冻结。

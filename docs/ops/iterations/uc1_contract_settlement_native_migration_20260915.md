...
| 来源 | 迁移前 | 迁移后 |
|---|---|---|
| 分类 8/10，seed 与 Python 模板 | `sections` 决定标题、顺序、字段归属 | 移除输出 `sections`；非结构字段策略逐项不变 |
...
字段帮助、标签、显隐／必填／只读 profiles、附件策略、动作说明、domain、ledger 元数据保留。旧配置没有独立的帮助或动作语义，不能把停用纯排序配置解释为删除业务能力。补充合同分类保留兼容章节，本批不减其消费者。

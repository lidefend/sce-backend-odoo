...
状态为 `RETIREMENT_PREPARED_AWAITING_EXACT_AUTHORIZATION`。本批完成受管入口、12 项精确 manifest、恢复 bundle、历史证据归档及两个独立技术任务的价值路由；**尚未删除任何本地或远端引用**。
...
8 个非零临时仓库测试 PASS：默认预演、SHA 漂移、开放 PR/工作树占用、远端缺失、未授权 apply、关联工作证据不可用、恢复包不完整和远端部分失败。真实仓库没有用于测试删除行为。
...
下一步仅是：以 manifest SHA-256 `cac22ce59dbabf3ce9eb77dcf356d7dc55b8c545473f2562f73d854e7bb754a7` 集中申请 12 项实际退役授权。授权后仍需立即重跑身份/开放 PR/工作树检查；漂移项逐项跳过，不扩大到四个保留分支或任何未列引用。

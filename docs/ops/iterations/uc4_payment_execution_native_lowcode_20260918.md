...
- 冻结候选（**以证据文件为准，本记录不内嵌自身提交哈希、也不枚举逐轮日志序号**，否则记录一动身份即失效）：
  `artifacts/fingerprints/uc4-g04-frozen.json` 的 `git_head`/`git_tree`/`digest`（7507 路径完整 tracked+untracked 指纹）＋
  `.git/codex/evidence/ci.local.quick/<git_head>.json` 的 exact-head Quick 回执。
  本 §8 属**冻结后文档补记**：补记提交本身会改变 HEAD，故最终身份一律按上述两个证据文件回读，不回读本节文字；
  每轮的 head/tree/digest/回执/日志与复核结论另在同目录证据包（`tmp/g04-evidence/`，已外部归档）中逐轮留档。
- `make ci.delivery.freeze.prepare` PASS（每轮冻结前生成证据均无未提交改动；日志 `artifacts/uc4-representative/freeze-prepare-g04-r*.log`；Quick 日志同目录 `quick-g04-r*.log`）。
- 身份沿革（每轮均为**纯文档/只读证据**变更，产品代码未变，页面证据按规则沿用不受影响）：
  ...
  `80099b7d…`（§8 去重「记录更正」段）→ `9fd337a8…`（补 §2.1 文件归属表）。**其后各轮同类纯文档/证据变更不再在本节逐条枚举**，
  完整链按 `tmp/g04-evidence/identity.json` 的 `identity_history` 回读。

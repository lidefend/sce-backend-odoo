...
- PR #468 的首次远端候选在 `frontend_release_gate` 失败；唯一根因是 X2Many 长值触发按钮的消费者类直接声明 `color: inherit`，违反 primitive adapter 对视觉属性所有权的约束。候选检查等待与 release candidate gate 均为该上游失败的连带结果。
- 修复把颜色表达移入既有 `ScButton appearance="structured-content"` 的通用主题规则；X2Many 消费者仅保留宽度、截断和文本布局。实现不含材料模型或字段名特判，不改变显示名、关系契约、业务动作或写入规则。
- 该提交使旧 `75ff4c64…` Quick receipt 与独立复核仅保留为历史证据；新 HEAD 必须重新绑定完整指纹、运行受影响非零 L2、聚焦长名称浏览器检查、完整 Quick、独立复核与精确候选远端检查后才可合并。

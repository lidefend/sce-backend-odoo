...
- 字段语义补项后的首次冻结 Quick 在末段责任映射守卫发现 `ui_contract_v2.py` 为 4358 行，超过既有 4312 行硬锁，因此失败且未签发 receipt；其余已执行门禁通过。恢复没有抬高预算：将业务策略字段可见性及新增结构权威判定下沉到既有 `ui_contract_v2_projection.py`，handler 回到 4312 行。责任映射守卫、L1 iteration、原生权威/旧回退/task 呈现 3 个契约方法及业务策略可见性 1 个方法均非零通过。一次错误的宿主包导入和一次 Odoo 标签尝试分别在收集期失败、执行 0 tests，明确不计入覆盖。该职责移动不改变最终契约、页面 DOM 或金额规则，既有 L3/L4 按确定性影响分析结转。

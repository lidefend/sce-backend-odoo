...
- 最终产品/P4 源候选：`b7f167b8873b0578850aee4e5b37fa3f082df7cd`，Tree `9e342fc9f924ee19759e7f0590a4c3dceb372dfa`，完整工作树指纹 `b11a76ea54eb62aa17043d148f0934e853db1713136851eab5d15aca0b7ddabc`。其包含已批准的 `85b05110…` 产品样板、独立复核发现的键盘焦点修复及其 P4 证据工具。
...
- 溢出浏览到首尾边界时按钮保持在 DOM 和焦点序列中，以 `aria-disabled` 表达不可继续滚动；共享 `ScButton` 通过公开 prop 保留调用方显式状态。
...
六个产品提交可独立回滚：`67301212`（A1 排版）、`d63a1dbb`（A2 表面）、`244cf29d`（A3 导航）、`85b05110`（只读字段边界）、`7b3121fe`（导航焦点连续性）、`30118588`（共享按钮显式 aria 状态）。`4bf81ada`、`b66f1640`、`b7f167b8` 仅为 P4 浏览器证据能力及适用范围修正。
...
| L4 light | `/home/lidefend/workspace/sce-offrepo/artifacts/playwright/tdesign-alignment-final-bound-light-b7f167b8/evidence-binding.json` | 1440×960、390×844 共 8 个页面/视口样本，`pass=true`、零写入、零错误；运行前后完整指纹逐字节一致，截图及摘要有 SHA-256 绑定 |
| L4 dark | `/home/lidefend/workspace/sce-offrepo/artifacts/playwright/tdesign-alignment-final-bound-dark-b7f167b8/evidence-binding.json` | 同一 8 个样本通过；长值、控件、表面、导航和横向内容可达，运行前后完整指纹逐字节一致 |
| L4 focus | `/home/lidefend/workspace/sce-offrepo/artifacts/playwright/tdesign-section-browse-focus-b66f1640/summary.json` 及上述两组绑定矩阵 | 1088×791 与 390×844 均验证正反向浏览；到达首尾时控制仍聚焦、仍连接 DOM 且正确禁用，不触发提交或业务写入 |
...
`85b05110…` 的原明暗矩阵完成人工产品复核，但摘要未绑定运行前后完整工作树指纹，现仅作历史产品/诊断材料。首次浅色矩阵在页面加载期间出现 `ERR_NETWORK_CHANGED` 和动态组件加载错误，摘要保留于 `tdesign-alignment-final-light-1440-390-85b05110`，归类为 `environment_defect`；通过证据没有覆盖或删除失败记录。为闭合候选身份，最终矩阵在 `b7f167b8…` 上按受管入口重新生成并绑定，而非把旧摘要冒充 L4 门禁。
...
首次独立复核还发现导航浏览按钮在边界处通过 `v-if` 被移除，键盘焦点会随节点消失。P0 改为保留控制并使用显式 `aria-disabled`；当共享 `ScButton` 覆盖调用方状态时，在公开 prop 层修复而非使用内部选择器。primitive adapter 27 项与 46 个组件审计、form canvas 7 项及严格类型检查通过。`37ccabc6…` 的 Quick 虽曾通过，但后续 P0 修复已使其失效，只保留为历史 receipt；最终 Quick 必须绑定新冻结 HEAD。
...
- 若需回滚，按 `30118588` → `7b3121fe` → `85b05110` → `244cf29d` → `d63a1dbb` → `67301212` 逆序回退；P4 探针可单独回退，没有业务数据回滚。

## 冻结策略

本文、英文报告、PR 草案及生成证据在冻结前一次提交。冻结后只运行一次 Quick，并由独立复核证明最终候选相对 `b7f167b8…` 仅有 P4 文档/生成证据变化；不为回填 SHA 或测试结果再修改候选。

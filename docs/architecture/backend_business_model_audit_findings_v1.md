...
The model count is not the issue. The current backend has 370 detected model classes, not "only 10 models". The 10 count is only the strict formal legacy-to-runtime fact set.
...
- projection registry entries: 34
...
The projection registry currently covers all 34 detected projection/read surfaces: 25 SQL views, 2 physical refresh tables, 3 controlled generated ledgers, 2 runtime workbench facts, 1 computed runtime summary, and 1 controlled writable snapshot.

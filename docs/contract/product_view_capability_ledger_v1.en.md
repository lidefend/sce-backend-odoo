...
The current structure baseline proves the resolved view but not the exact contributor that originated each node. When origin is not proven, `origin_status=unproven`, `origin_view_ref` is empty, and the native-stage `CAPABILITY_NATIVE_OCCURRENCE_ORIGIN_UNPROVEN` reason applies; inventing the root view as origin is forbidden. Reason priority is fixed as `native -> normalized -> semantic -> frontend -> interaction`. The terminal state describes final deliverability while the earliest loss selects its reason code; an atom with unproven origin and no later carrier remains `unsupported` but reports the native-stage reason.

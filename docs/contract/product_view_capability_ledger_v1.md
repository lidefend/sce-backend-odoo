...
每份台账必须绑定既有 `codex_complete_worktree_fingerprint/v1` 完整候选指纹、Git HEAD、基线 SHA、范围清单哈希，以及数据库架构政策、正式菜单政策、原因码注册表、taxonomy、normalized map、frontend map、同候选视图结构与 carrier evidence、模块版本集合、用户、公司、语言和权限组。运行身份固定为 `local.clean` / `sc-local-clean` / `sc_clean` / `^sc_clean$` 且 `demo_data=false`，不得使用 Git commit SHA 代替完整指纹，也不得使用手工拼装的 Compose、数据库或凭据代替。运行 evidence 固定引用 `artifacts/contract` 下由受治理 Make target 生成的候选文件，不复制为大型 tracked baseline，也不得引用另一候选的历史 artifact。

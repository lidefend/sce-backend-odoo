...
.PHONY: release.production.first_fresh.cleanup.preflight release.production.first_fresh.cleanup.confirm release.production.first_fresh.cleanup release.production.failed_deployment.cleanup.preflight release.production.failed_deployment.cleanup.confirm release.production.failed_deployment.cleanup release.production.admin.harden release.production.admin_identity.baseline release.production.formal_modules.install_missing
...
verify.production.release_contract:
	...
	@python3 scripts/release/test_production_failed_deployment_cleanup.py
...
release.production.failed_deployment.cleanup.preflight:
	@test "$(ENV)" = "prod" || (echo "ENV=prod is required"; exit 2)
	@test "$(PRODUCTION_COMPOSE_PROJECT)" = "sc_production" || (echo "PRODUCTION_COMPOSE_PROJECT must be sc_production"; exit 2)
	@test "$(TARGET_DB)" = "sc_production" || (echo "TARGET_DB must be sc_production"; exit 2)
	@test -n "$(EXPECTED_FAILED_IMAGE_ID)" || (echo "EXPECTED_FAILED_IMAGE_ID is required"; exit 2)
	@test -n "$(EXPECTED_FAILED_DEPLOYMENT_TOOL_SHA)" || (echo "EXPECTED_FAILED_DEPLOYMENT_TOOL_SHA is required"; exit 2)
	@test -n "$(FAILED_CLEANUP_EVIDENCE)" || (echo "FAILED_CLEANUP_EVIDENCE is required"; exit 2)
	@python3 scripts/release/production_failed_deployment_cleanup.py plan

release.production.failed_deployment.cleanup.confirm:
	@test "$(CONFIRM_FAILED_DEPLOYMENT_CLEANUP)" = "YES_REMOVE_VERIFIED_FAILED_SC_PRODUCTION_ATTEMPT" || \
		(echo "exact failed deployment cleanup confirmation is required"; exit 2)
	@test -n "$(EXPECTED_CLEANUP_PLAN_SHA256)" || (echo "EXPECTED_CLEANUP_PLAN_SHA256 is required"; exit 2)

release.production.failed_deployment.cleanup: guard.prod.danger release.production.failed_deployment.cleanup.confirm
	@python3 scripts/release/production_failed_deployment_cleanup.py apply

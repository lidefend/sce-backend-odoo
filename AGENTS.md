...
- The default inner-loop static entry is `make ci.local.iteration`, followed by exactly the affected non-zero L2 target and, only when needed, a focused `local.dev.*` runtime check. This L1 entry is not delivery evidence. `make ci.local.quick` is reserved for a clean frozen delivery HEAD and must not be used after every edit or local commit.

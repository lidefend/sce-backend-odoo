...
- Reuse the current batch inventory and result index across commands and resumed turns. Recheck only identities or dependencies affected by an observed change; this requirement does not authorize repeating a full repository, menu or environment inventory before every edit.
- Business topics must consume existing governed infrastructure. They must not derive or create a Compose project, database, port, volume, credential file, fixture system, or runtime profile. Within an already authorized acceptance scope, minimal P4 extensions to existing test tools and local.dev wrappers are authorized when they reuse the registered environment, credentials and data authority, preserve gates, bind the exact target scope, and provide restoration checks for test configuration writes. Record the owning layer and run focused tests; do not request redundant approval. New environment or credential authority still requires separately authorized P4 work.

...
run_check "S66 business entity exists without migration carriers" "s66_ledger_entity_surface" \
  "select case when count(*) = 1 then 'ok' else 'S66 business entity missing' end from sc_business_entity where name='S66 德阳项目经营载体' and entity_type='project_carrier' and active=true;" \
  "select id, name, entity_type, company_id, partner_id, active from sc_business_entity where name='S66 德阳项目经营载体';"
...
run_check "S72 formal project member assignment exists" "s72_project_governance_surface" \
  "select case when count(*) = 1 then 'ok' else 'S72 project member assignment missing' end from sc_project_member_assignment where project_id in (select res_id from ir_model_data where module='smart_construction_demo' and name='sc_demo_project_065_cost_funding') and user_id=1 and source='manual' and active=true;" \
  "select id, project_id, user_id, source, note, active from sc_project_member_assignment where project_id in (select res_id from ir_model_data where module='smart_construction_demo' and name='sc_demo_project_065_cost_funding');"
...
run_check "S74 supplier type and partner records exist" "s74_partner_supplier_surface" \
  "select case when (select count(*) from sc_supplier_type where code in ('service','equipment') and active is true) = 2 and (select count(*) from res_partner where name='S74 四川智建咨询服务有限公司' and supplier_rank > 0 and customer_rank > 0 and sc_supplier_type='service') = 1 then 'ok' else 'S74 supplier partner records missing' end;" \
...
run_check "S79 independent location and contract dimensions exist" "s79_execution_structure_surface" \
  "select case when (select count(*) from construction_location_breakdown where code in ('S79-SITE','S79-BUILDING','S79-FIRE-ZONE')) = 3 and (select count(*) from construction_location_breakdown zone join construction_location_breakdown building on building.id=zone.parent_id join construction_location_breakdown site on site.id=building.parent_id where zone.code='S79-FIRE-ZONE' and building.code='S79-BUILDING' and site.code='S79-SITE') = 1 and (select count(*) from construction_contract_section where code='S79-SECTION-FIRE') = 1 then 'ok' else 'S79 execution dimensions missing' end;" \
  "select id, code, name, location_type, parent_id from construction_location_breakdown where code like 'S79-%' order by parent_path, sequence;"
run_check "S79 BOQ is allocated to execution scope" "s79_execution_structure_surface" \
  "select case when (select count(*) from project_boq_allocation a join project_boq_line b on b.id=a.boq_line_id join construction_execution_scope s on s.id=a.execution_scope_id where b.code='S79-BOQ-FIRE-PIPE' and s.location_id in (select id from construction_location_breakdown where code='S79-FIRE-ZONE') and s.contract_section_id in (select id from construction_contract_section where code='S79-SECTION-FIRE') and s.wbs_id is not null and a.allocation_ratio=100.0 and b.amount=59200.0) = 1 then 'ok' else 'S79 BOQ execution allocation missing' end;" \
  "select b.code, b.quantity, b.amount, a.allocation_ratio, s.wbs_id, s.location_id, s.contract_section_id from project_boq_allocation a join project_boq_line b on b.id=a.boq_line_id join construction_execution_scope s on s.id=a.execution_scope_id where b.code='S79-BOQ-FIRE-PIPE';"

...
BATCH_BINDINGS = {
    ...
    "frontend/apps/web/src/pages/contractForm/ContractFormDriverHost.vue": {"scerrorstate": {"attrs": {"v-if": "error || !renderModel"}, "minimum": 1}, "scinlinestate": {"states": {"empty"}, "attrs": {"class": "canonical-form-activity-empty", "v-else": ""}, "minimum": 1}},
    ...
    "p0-form-relation-workflow-completion-v1": {
        ...
        "frontend/apps/web/src/components/professional-fields/ProfessionalDetailCollectionControl.vue": {
            "div": {"attrs": {"data-semantic-component": "ProfessionalDetailCollectionControl", ":data-control-state": "field.readonly ? 'readonly' : 'editable'"}},
            "scdisclosure": {"import": "ScDisclosure", "attrs": {"v-if": "optionalPresentation?.render", ":open": "optionalPresentation.open"}},
            "scinlinestate": {"states": {"info"}, "attrs": {"v-if": "optionalPresentation.linkedAmountMessage", ":label": "optionalPresentation.linkedAmountMessage"}},

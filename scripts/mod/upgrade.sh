#!/usr/bin/env bash
set -euo pipefail
: "${SC_GOVERNED_MODULE_LIFECYCLE_ENTRY:?DENY: use a governed module lifecycle Make target; direct module upgrade is forbidden}"

ROOT_DIR="${ROOT_DIR:-$(cd "$(dirname "$0")/../.." && pwd)}"
export ROOT_DIR
source "$ROOT_DIR/scripts/common/governed_make_entry.sh"
require_governed_make_ancestor "module upgrade" "$ROOT_DIR" "mod.upgrade,acceptance.module.upgrade,acceptance.baseline.upgrade,prod.upgrade.core"

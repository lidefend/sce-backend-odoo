...
INTERNAL_USER_XMLID = "base.group_user"


def _required(name):
    value = str(os.environ.get(name, "") or "").strip()
    if not value:
        raise UserError(f"TPV1_REQUIRED_ENV_MISSING:{name}")
    return value


def _required_xmlid_list(name):
    try:
        values = json.loads(_required(name))
...
if (
    ...
    or required_existing_xmlids != (INTERNAL_USER_XMLID,)
    or declared_closure_xmlids
...
data_operator = env.ref(DATA_OPERATOR_XMLID, raise_if_not_found=False)
data_operator_before = bool(
    data_operator and data_operator.id in before_groups
)
...
actual_addition_xmlids = _group_xmlids(after_groups - before_groups)
...
if DATA_OPERATOR_XMLID in actual_addition_xmlids:
    raise UserError("TPV1_IMPORT_OPERATOR_DATA_OPERATOR_FORBIDDEN")
data_operator_after = bool(data_operator and data_operator.id in after_groups)
if data_operator_after != data_operator_before:
    raise UserError("TPV1_IMPORT_OPERATOR_DATA_OPERATOR_FORBIDDEN")

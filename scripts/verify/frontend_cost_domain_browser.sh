...
resolve_target() {
  DB_NAME="$DB_NAME" bash "$ROOT_DIR/scripts/ops/odoo_shell_exec.sh" \
    < "$ROOT_DIR/scripts/verify/local_dev_cost_domain_ids.py" \
    | sed -n 's/^LOCAL_DEV_COST_DOMAIN_JSON=//p' | tail -1
}

before="$(resolve_target)"
[[ -n "$before" ]] || { echo "cost domain browser target resolution failed" >&2; exit 1; }
set +e
FRONTEND_COST_DOMAIN_TARGET="$before" FRONTEND_URL="${FRONTEND_URL:-http://127.0.0.1:${NGINX_PORT}}" \
DB_NAME="$DB_NAME" E2E_PASSWORD="$SC_DEMO_USER_PASSWORD" CANDIDATE_HEAD="${CANDIDATE_GIT_HEAD:-$(git -C "$ROOT_DIR" rev-parse HEAD)}" \
node "$ROOT_DIR/scripts/verify/frontend_cost_domain_browser.mjs"
browser_status=$?
set -e
...
if before["business_fingerprint"] != after["business_fingerprint"]:
    raise SystemExit("cost domain browser changed governed business data")
print("[frontend.cost-domain.browser] business fingerprints unchanged")
PY
exit "$browser_status"

...
class ProfessionalRelationFieldGuardTests(unittest.TestCase):
    ...
    def test_many2one_command_cannot_regress_to_private_button(self):
        ...
        self.assertTrue(any("four shared ScButton" in item for item in validate(read_text)))

    def test_many2one_option_remains_a_native_option(self):
        def read_text(path):
            value = (ROOT / path).read_text(encoding="utf-8")
            return value.replace('<button\n                          v-for="(option, optionIndex)', '<ScButton\n                          v-for="(option, optionIndex)', 1)
        self.assertTrue(any("native option" in item for item in validate(read_text)))

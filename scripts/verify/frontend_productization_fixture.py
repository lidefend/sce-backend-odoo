...
work_settlement = env.ref(
    "smart_construction_acceptance_fixture.fe_b05_work_settlement_a",
    raise_if_not_found=False,
)
if not (
    work_settlement
    and work_settlement.state == "approve"
    and str(work_settlement.settlement_period_start) == "2026-07-01"
    and str(work_settlement.settlement_period_end) == "2026-07-31"
    and work_settlement.submitted_amount == 200.0
    and work_settlement.approved_amount == 200.0
    and "审定结算金额" in (work_settlement.contract_id.contract_payment_method_text or "")
):
    fail("PFL-035 create/save/reopen source facts are not deterministic")

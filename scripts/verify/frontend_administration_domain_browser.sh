...
resolve_target() {
  DB_NAME="$DB_NAME" bash "$ROOT_DIR/scripts/ops/odoo_shell_exec.sh" \
    < "$ROOT_DIR/scripts/verify/local_dev_administration_domain_ids.py" \
    | sed -n 's/^LOCAL_DEV_ADMINISTRATION_DOMAIN_JSON=//p' | tail -1
}
before="$(resolve_target)"
[[ -n "$before" ]] || { echo "administration browser target resolution failed" >&2; exit 1; }
set +e
FRONTEND_ADMINISTRATION_DOMAIN_TARGET="$before" \
...
node "$ROOT_DIR/scripts/verify/frontend_administration_domain_browser.mjs"
browser_status=$?
set -e
...
if before["business_fingerprint"] != after["business_fingerprint"]:
    raise SystemExit("administration browser changed governed business data")
print("[frontend.administration-domain.browser] business fingerprints unchanged")
PY
exit "$browser_status"

...
const target = JSON.parse(process.env.FRONTEND_ADMINISTRATION_DOMAIN_TARGET || '{}');
const frontendUrl = process.env.FRONTEND_URL || '';
const database = process.env.DB_NAME || '';
const password = process.env.E2E_PASSWORD || '';
const outputDir = path.resolve('artifacts/playwright/phase10-administration-domain');
...
check(frontendUrl && database && password, 'administration browser identity is incomplete');
check(target?.user?.login && target?.security_user?.login, 'administration users are missing', target);
fs.mkdirSync(outputDir, { recursive: true });
const browser = await launchChromium({ headless: true });
const blankEvidence = () => ({ errors: [], mutations: [], contracts: [], contractRequests: [] });
const report = {
  schemaVersion: 'frontend_administration_domain_browser.v1',
...
try {
  ...
  check(report.primary.result.model === 'hr.department', 'administration model identity drifted', report.primary.result);
  check(String(report.primary.result.actionId) === String(actionId), 'administration action identity drifted', report.primary.result);
  check(String(report.primary.result.menuId) === String(menuId), 'administration menu identity drifted', report.primary.result);
  check(report.primary.result.h1 === 1 && report.primary.result.headers === 1, 'administration page identity is not unique', report.primary.result);
  check(report.primary.result.tables === 1 || report.primary.result.emptyStates === 1, 'administration list has no professional state', report.primary.result);
  check(report.primary.errors.length === 0 && report.primary.mutations.length === 0, 'administration primary journey is not clean', report.primary);
  await page.screenshot({ path: path.join(outputDir, 'department-list.png'), fullPage: true });
  ...
  check(report.security.result.denialVisible, 'unauthorized administration entry did not fail closed', report.security.result);
  check(report.security.errors.length === 0 && report.security.mutations.length === 0, 'administration security journey is not clean', report.security);

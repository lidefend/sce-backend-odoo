...
for key in ("business_fingerprint", "actionable_fingerprint"):
    if before[key] != after[key]:
        raise SystemExit("payment domain browser changed governed business data: %s" % key)
print("[frontend.payment-domain.browser] business fingerprints unchanged")
PY
exit "$browser_status"

...
const target = JSON.parse(process.env.FRONTEND_FINANCE_CENTER_TARGET || '{}');
const frontendUrl = process.env.FRONTEND_URL || '';
const database = process.env.DB_NAME || '';
const password = process.env.E2E_PASSWORD || '';
const outputDir = path.resolve('artifacts/playwright/phase10-finance-center');
...
check(frontendUrl && database && password, 'finance browser identity is incomplete');
check(target?.user?.login && target?.security_user?.login, 'finance users are missing', target);
fs.mkdirSync(outputDir, { recursive: true });
const browser = await launchChromium({ headless: true });
const blankEvidence = () => ({ errors: [], mutations: [], contracts: [], contractRequests: [] });
const report = {
  schemaVersion: 'frontend_finance_center_browser.v1',
...
try {
  ...
  check(report.primary.result.model === 'sc.receipt.income', 'finance model identity drifted', report.primary.result);
  check(String(report.primary.result.actionId) === String(actionId), 'finance action identity drifted', report.primary.result);
  check(String(report.primary.result.menuId) === String(menuId), 'finance menu identity drifted', report.primary.result);
  check(report.primary.result.h1 === 1 && report.primary.result.headers === 1, 'finance page identity is not unique', report.primary.result);
  check(report.primary.result.tables === 1 || report.primary.result.emptyStates === 1, 'finance list has no professional state', report.primary.result);
  check(report.primary.errors.length === 0 && report.primary.mutations.length === 0, 'finance primary journey is not clean', report.primary);
  await page.screenshot({ path: path.join(outputDir, 'receipt-income-list.png'), fullPage: true });
  ...
  check(report.security.result.denialVisible, 'unauthorized finance entry did not fail closed', report.security.result);
  check(report.security.errors.length === 0 && report.security.mutations.length === 0, 'finance security journey is not clean', report.security);
  await securityPage.screenshot({ path: path.join(outputDir, 'e2e-smoke-role-denied.png'), fullPage: true });

...
const target = JSON.parse(process.env.FRONTEND_TAX_CENTER_TARGET || '{}');
const frontendUrl = process.env.FRONTEND_URL || '';
const database = process.env.DB_NAME || '';
const password = process.env.E2E_PASSWORD || '';
const outputDir = path.resolve('artifacts/playwright/phase10-tax-center');
...
check(frontendUrl && database && password, 'tax browser identity is incomplete');
check(target?.user?.login && target?.security_user?.login, 'tax users are missing', target);
fs.mkdirSync(outputDir, { recursive: true });
const browser = await launchChromium({ headless: true });
const blankEvidence = () => ({ errors: [], mutations: [], contracts: [], contractRequests: [] });
const report = {
  schemaVersion: 'frontend_tax_center_browser.v1', head: process.env.CANDIDATE_HEAD || '', target,
...
try {
  ...
  check(report.primary.result.model === 'sc.invoice.registration', 'tax model identity drifted', report.primary.result);
  check(String(report.primary.result.actionId) === String(actionId), 'tax action identity drifted', report.primary.result);
  check(String(report.primary.result.menuId) === String(menuId), 'tax menu identity drifted', report.primary.result);
  check(report.primary.result.h1 === 1 && report.primary.result.headers === 1, 'tax page identity is not unique', report.primary.result);
  check(report.primary.result.tables === 1 || report.primary.result.emptyStates === 1, 'tax list has no professional state', report.primary.result);
  check(report.primary.errors.length === 0 && report.primary.mutations.length === 0, 'tax primary journey is not clean', report.primary);
  await page.screenshot({ path: path.join(outputDir, 'input-invoice-list.png'), fullPage: true });
  ...
  check(report.security.result.denialVisible, 'unauthorized tax entry did not fail closed', report.security.result);
  check(report.security.errors.length === 0 && report.security.mutations.length === 0, 'tax security journey is not clean', report.security);

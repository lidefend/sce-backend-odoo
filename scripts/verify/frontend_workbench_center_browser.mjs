...
const target = JSON.parse(process.env.FRONTEND_WORKBENCH_CENTER_TARGET || '{}');
const frontendUrl = process.env.FRONTEND_URL || '';
const database = process.env.DB_NAME || '';
const password = process.env.E2E_PASSWORD || '';
const outputDir = path.resolve('artifacts/playwright/phase10-workbench-center');
...
check(frontendUrl && database && password, 'workbench browser identity is incomplete');
check(target?.user?.login && target?.action?.id && target?.menu?.id, 'workbench target is incomplete', target);
fs.mkdirSync(outputDir, { recursive: true });
const browser = await launchChromium({ headless: true });
const blankEvidence = () => ({ errors: [], mutations: [], contracts: [], contractRequests: [] });
const report = {
  schemaVersion: 'frontend_workbench_center_browser.v1',
...
try {
  ...
  check(report.primary.result.model === 'sc.workbench.item', 'workbench model identity drifted', report.primary.result);
  check(String(report.primary.result.actionId) === String(actionId), 'workbench action identity drifted', report.primary.result);
  check(String(report.primary.result.menuId) === String(menuId), 'workbench menu identity drifted', report.primary.result);
  check(report.primary.result.h1 === 1 && report.primary.result.headers === 1, 'workbench page identity is not unique', report.primary.result);
  check(report.primary.result.tables === 1 || report.primary.result.emptyStates === 1, 'workbench list has no professional state', report.primary.result);
  check(report.primary.errors.length === 0 && report.primary.mutations.length === 0, 'workbench primary journey is not clean', report.primary);
  await page.screenshot({ path: path.join(outputDir, 'task-center-list.png'), fullPage: true });
  ...
  await securityPage.waitForURL((url) => url.pathname.includes('/login'), { timeout: 45000 });
  report.security.result = { url: securityPage.url(), loginRequired: true };
  check(report.security.errors.length === 0 && report.security.mutations.length === 0, 'workbench anonymous denial is not clean', report.security);
  await securityPage.screenshot({ path: path.join(outputDir, 'anonymous-login-required.png'), fullPage: true });

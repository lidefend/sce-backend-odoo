...
from scripts.verify.frontend_native_form_action_presentation_guard import RENDERER, SMART_ACTION, validate
...
class NativeFormActionPresentationGuardTest(unittest.TestCase):
    ...
    def test_repository_contract_passes(self):
        self.assertEqual(validate(self.source, self.smart_action), [])
    ...
    def test_ordinary_action_cannot_regress_to_private_button(self):
        ...
        self.assertTrue(any("ordinary action" in error or "expected two" in error for error in validate(altered, self.smart_action)))

    def test_private_hover_presentation_is_rejected(self):
        altered = f"{self.source}\n.native-action-btn:hover {{ background: red; }}"
        self.assertIn("native ordinary actions must not override ScButton appearance or states", validate(altered, self.smart_action))
    ...
    def test_action_event_authority_is_preserved(self):
        ...
        self.assertTrue(any("emitNativeAction(node)" in error for error in validate(altered, self.smart_action)))
    ...
    def test_stateful_controls_are_not_mechanically_replaced(self):
        ...
        self.assertTrue(any("stateful" in error for error in validate(altered, self.smart_action)))

    def test_smart_action_cannot_return_to_renderer_private_css(self):
        altered = f"{self.source}\n.native-action-btn--smart {{ color: red; }}"
        self.assertIn("native renderer must not retain parallel smart-action appearance", validate(altered, self.smart_action))
    ...
    def test_smart_action_semantic_identity_is_required(self):
        ...
        self.assertTrue(any("smart action" in error for error in validate(self.source, altered)))

...
const target = JSON.parse(process.env.FRONTEND_REPORTING_CENTER_TARGET || '{}');
const frontendUrl = process.env.FRONTEND_URL || '';
const database = process.env.DB_NAME || '';
const password = process.env.E2E_PASSWORD || '';
const outputDir = path.resolve('artifacts/playwright/phase10-reporting-center');
...
check(frontendUrl && database && password, 'reporting browser identity is incomplete');
check(target?.user?.login && target?.security_user?.login, 'reporting users are missing', target);
check(target.user_menu_visible === true, 'reporting primary menu authority is unavailable', target);
check(target.security_menu_visible === false, 'reporting security principal can access the primary menu', target);
fs.mkdirSync(outputDir, { recursive: true });
const browser = await launchChromium({ headless: true });
const blankEvidence = () => ({ errors: [], mutations: [], contracts: [], contractRequests: [] });
const report = {
  schemaVersion: 'frontend_reporting_center_browser.v1', head: process.env.CANDIDATE_HEAD || '', target,
...
try {
  ...
  const securityActionId = Number(target.security_action.id);
  const securityMenuId = Number(target.security_menu.id);
  ...
  try {
    ...
    await page.screenshot({ path: path.join(outputDir, 'primary-readiness-failure.png'), fullPage: true });
    throw error;
  }
  ...
  check(report.primary.result.model === 'sc.operating.metrics.project', 'reporting model identity drifted', report.primary.result);
  check(String(report.primary.result.actionId) === String(actionId), 'reporting action identity drifted', report.primary.result);
  check(String(report.primary.result.menuId) === String(menuId), 'reporting menu identity drifted', report.primary.result);
  check(report.primary.result.h1 === 1 && report.primary.result.headers === 1, 'reporting page identity is not unique', report.primary.result);
  check(report.primary.result.tables === 1 || report.primary.result.emptyStates === 1, 'reporting list has no professional state', report.primary.result);
  check(report.primary.errors.length === 0 && report.primary.mutations.length === 0, 'reporting primary journey is not clean', report.primary);
  await page.screenshot({ path: path.join(outputDir, 'project-operation-report-list.png'), fullPage: true });
  ...
  await securityPage.goto(`${frontendUrl}/a/${securityActionId}?menu_id=${securityMenuId}`, { waitUntil: 'domcontentloaded', timeout: 45000 });
  ...
  try {
    ...
    report.security.failure = {
      url: securityPage.url(),
      body: (await securityPage.locator('body').innerText()).slice(0, 2000),
      message: error instanceof Error ? error.message : String(error),
    };
    await securityPage.screenshot({ path: path.join(outputDir, 'security-readiness-failure.png'), fullPage: true });
    throw error;
  }
  report.security.result = {
    url: securityPage.url(), actionId: securityActionId, menuId: securityMenuId,
    denialVisible: (await denial.innerText()).includes('访问受限'),
  };
  check(report.security.result.denialVisible, 'unauthorized reporting entry did not fail closed', report.security.result);
  check(report.security.errors.length === 0 && report.security.mutations.length === 0, 'reporting security journey is not clean', report.security);
  await securityPage.screenshot({ path: path.join(outputDir, 'project-role-denied.png'), fullPage: true });

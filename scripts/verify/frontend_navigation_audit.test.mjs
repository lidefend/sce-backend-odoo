...
const manifest = {
  ...
  roles: { finance: { expected_count: 2, leaf_keys: [keyA, keyB] } },
};
const row = (key) => {
...
assert.equal(compareNavigation(manifest, [row(keyA), row(keyB)], ['fixture_role_finance']).total.result, 'PASS');
const missing = compareNavigation(manifest, [row(keyA)], ['fixture_role_finance']);
assert.equal(missing.total.result, 'FAIL');
assert.deepEqual(missing.roles.finance.missing_leaf_keys, [keyB]);

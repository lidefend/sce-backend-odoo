#!/usr/bin/env python3
"""Create deterministic checked-in contract-domain rollout evidence."""
...
markdown = _SHARED.markdown
normalized_snapshot = _SHARED.normalized_snapshot
...
def main() -> int:
    ...
    Path(args.markdown_output).write_text(
        markdown(snapshot, "Contract Domain Frontend Rollout v1"),
        encoding="utf-8",
    )
    print(
        json.dumps(
            {"status": snapshot["status"], "actions": len(snapshot["actions"])},
            ensure_ascii=False,
        )
    )

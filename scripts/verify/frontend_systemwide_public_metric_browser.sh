...
resolve_target() {
  DB_NAME="$DB_NAME" bash "$ROOT_DIR/scripts/ops/odoo_shell_exec.sh" \
    < "$ROOT_DIR/scripts/verify/local_dev_systemwide_public_metric_ids.py" \
    | sed -n 's/^LOCAL_DEV_SYSTEMWIDE_PUBLIC_METRIC_JSON=//p' | tail -1
}

before="$(resolve_target)"
[[ -n "$before" ]] || { echo "systemwide public-metric target resolution failed" >&2; exit 1; }
set +e
FRONTEND_SYSTEMWIDE_PUBLIC_METRIC_TARGET="$before" \
...
node "$ROOT_DIR/scripts/verify/frontend_systemwide_public_metric_browser.mjs"
browser_status=$?
set -e
...
if before["businessFingerprint"] != after["businessFingerprint"]:
    raise SystemExit("systemwide public-metric browser changed governed business data")
print("[frontend.systemwide-public-metric.browser] business fingerprint unchanged")
PY
exit "$browser_status"

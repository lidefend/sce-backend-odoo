...
def main() -> int:
    ...
    if (
        "runtimeOccurrenceState:(node:NativeFormLayoutNode)" not in field_schema_source
        or "const state=context.runtimeOccurrenceState(node)" not in field_schema_source
        or "widgetStatusById.get(widgetId)" not in form_layout_source
        or "V2_OCCURRENCE_STATUS_MISSING" not in form_layout_source
    ):
        errors.append("web contract form page must consume exact occurrence status and fail closed when it is missing")

...
const target = JSON.parse(process.env.FRONTEND_BASE_CONFIGURATION_DOMAIN_TARGET || '{}');
const frontendUrl = process.env.FRONTEND_URL || '';
const database = process.env.DB_NAME || '';
const password = process.env.E2E_PASSWORD || '';
const outputDir = path.resolve('artifacts/playwright/phase10-base-configuration-domain');
...
check(frontendUrl && database && password, 'base-configuration browser identity is incomplete');
check(target?.user?.login && target?.security_user?.login, 'base-configuration users are missing', target);
fs.mkdirSync(outputDir, { recursive: true });
const browser = await launchChromium({ headless: true });
const blankEvidence = () => ({ errors: [], mutations: [], contracts: [], contractRequests: [] });
const report = {
  schemaVersion: 'frontend_base_configuration_domain_browser.v1',
...
try {
  ...
  check(report.primary.result.model === 'ui.business.config.contract', 'base-configuration model identity drifted', report.primary.result);
  check(String(report.primary.result.actionId) === String(actionId), 'base-configuration action identity drifted', report.primary.result);
  check(String(report.primary.result.menuId) === String(menuId), 'base-configuration menu identity drifted', report.primary.result);
  check(report.primary.result.h1 === 1 && report.primary.result.headers === 1, 'base-configuration page identity is not unique', report.primary.result);
  check(report.primary.result.tables === 1 || report.primary.result.emptyStates === 1, 'base-configuration list has no professional state', report.primary.result);
  check(report.primary.errors.length === 0 && report.primary.mutations.length === 0, 'base-configuration primary journey is not clean', report.primary);
  await page.screenshot({ path: path.join(outputDir, 'form-configuration-list.png'), fullPage: true });
  ...
  check(report.security.result.denialVisible, 'unauthorized base-configuration entry did not fail closed', report.security.result);
  check(report.security.errors.length === 0 && report.security.mutations.length === 0, 'base-configuration security journey is not clean', report.security);

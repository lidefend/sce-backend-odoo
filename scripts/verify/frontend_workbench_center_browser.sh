...
resolve_target() {
  DB_NAME="$DB_NAME" bash "$ROOT_DIR/scripts/ops/odoo_shell_exec.sh" \
    < "$ROOT_DIR/scripts/verify/local_dev_workbench_center_ids.py" \
    | sed -n 's/^LOCAL_DEV_WORKBENCH_CENTER_JSON=//p' | tail -1
}
before="$(resolve_target)"
[[ -n "$before" ]] || { echo "workbench browser target resolution failed" >&2; exit 1; }
set +e
FRONTEND_WORKBENCH_CENTER_TARGET="$before" \
...
node "$ROOT_DIR/scripts/verify/frontend_workbench_center_browser.mjs"
browser_status=$?
set -e
...
if before["business_fingerprint"] != after["business_fingerprint"]:
    raise SystemExit("workbench browser changed governed business data")
print("[frontend.workbench-center.browser] business fingerprints unchanged")
PY
exit "$browser_status"

...
async function requestCreateSaveReopenJourney(browser, page) {
  ...
  for (const fact of ['付款申请', 'FE Project A', 'FE-A Counterparty', 'FE-A Contract', '2026-07-01', '2026-07-31']) {
  ...
  const fileInput = page.locator('[data-field-name="attachment_ids"] input[type="file"]').first();
...
async function listSearchFilterPageAndReturnJourney(page) {
  const assertPaymentAmountAggregate = async (data, label) => {
    ...
    check(
      body.includes('当页总计') && body.includes('总计'),

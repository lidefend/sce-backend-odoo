...
professional_m2m = read("components/professional-fields/ProfessionalManyToManySelect.vue")
...
require(header, ':disabled="busy"', "busy workflow control is not interactive")
require(header, ':readonly="statusbar.readonly"', "readonly workflow is not interactive")
...
if '>只读</span>' in fields:
    raise SystemExit("[frontend_detail_form_productization_guard] FAIL generic readonly label must not repeat field state")
require(fields, ':data-field-state="fieldState(field)"', "machine-readable readonly state")
require(fields, "v-else-if=\"fieldConfigEditable\"", "field label editor isolation")
require(fields, "displayFields", "readonly empty scalar fact suppression")
require(fields, "!props.preferReadonlyFacts || props.fieldSelectionMode || props.fieldConfigEditable", "readonly empty suppression scope")
require(fields, "field.type === 'one2many'", "readonly detail collection empty-state preservation")
require(fields, "displayFields.length || fieldSelectionMode || fieldConfigEditable || slots.action || hint", "empty readonly card suppression")
...
require(header, "grid-template-columns: max-content minmax(12rem, 1fr)", "content-driven workflow step width")
...
require(x2many, "<ProfessionalManyToManySelect", "professional many2many delegation")
require(professional_m2m, 'v-if="field.readonly" class="m2m-readonly"', "readonly many2many facts")
native_tree = read("components/template/NativeFormTreeRenderer.vue")
native_bridge = read("pages/contractForm/canonicalNativeFormBridge.ts")
require(native_bridge, "tier: action.tier", "canonical action tier projection")
require(native_tree, "'data-action-tier': String(action.tier", "native primary action evidence")
object_task_page = read("pages/contractForm/ObjectTaskPage.vue")
require(object_task_page, ".object-task-page__current-task-actions {\n    position: fixed;", "mobile task priority")
require(object_task_page, "grid-template-columns: repeat(2, minmax(0, 1fr));", "mobile summary density")
require(object_task_page, "import ScInlineState", "professional feedback component")
...
require(object_task_page, 'data-contract-classification-gap="unclassified-readonly-fields"', "unclassified readonly visibility boundary")

...
def validate(list_source: str | None = None, footer_source: str | None = None, css_source: str | None = None, visual_source: str | None = None, presentation_source: str | None = None) -> list[str]:
    ...
    if list_text.count('<CollectionAggregateFooter') != 1 or list_text.count(':foot-data="collectionFootData(') != 2:
        failures.append("collection list must expose TDesign flat/group footData and one mobile summary adapter")
...
if __name__ == "__main__":
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)
    print("[frontend_collection_aggregate_footer_guard] PASS owners=1 adapters=tdesign-footData,mobile-summary scopes=page,total")

"""Resolve the governed local.dev product-configuration target without writes."""
...
if not user.active or not user.has_group(
    ...
    raise RuntimeError("governed base-configuration principal is unavailable")
security_user = env.ref("smart_construction_demo.user_demo_role_finance")
menu = env.ref("smart_construction_core.menu_sc_business_config_workbench")
action = env.ref("smart_construction_core.action_sc_business_config_workbench")
if menu.action != action or action.res_model != "ui.business.config.contract":
    raise RuntimeError("base-configuration menu/action authority mismatch")

record_env = (
    env["ui.business.config.contract"]
    .with_user(user)
...
print(
    "LOCAL_DEV_BASE_CONFIGURATION_DOMAIN_JSON=%s"

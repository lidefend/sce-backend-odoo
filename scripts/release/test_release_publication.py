...
class PublicationContractTests(unittest.TestCase):
    ...
    def test_customer_real_plan_evidence_is_not_part_of_product_publication(self):
        self.assertFalse((self.candidate / "final-image-real-plan.json").exists())
        result = self.pipeline().execute()
        self.assertEqual(result["state"], "PUBLICATION_COMPLETE")
        self.assertNotIn("final_image_real_plan_sha256", result["identity"])

...
// Dynamic menu components are backend-controlled identifiers. Keep the
// registry limited to production runtimes so Starter demo pages cannot be
// reached accidentally or pulled into the release bundle.
const productionDynamicViews = import.meta.glob([
  '../../pages/odoo/action/index.vue',
  '../../pages/SceneRuntimeView.vue',
]);

// 动态引入路由组件
function asyncImportRoute(routes: RouteItem[] | undefined) {
  dynamicViewsModules = dynamicViewsModules || productionDynamicViews;
  if (!routes) return;
...
export function transformObjectToRoute<T = RouteItem>(routeList: RouteItem[]): T[] {
  routeList.forEach(async (route) => {
    ...
    if (component) {
      if (['LAYOUT', 'BLANK'].includes(component.toUpperCase())) {

  ...
  <t-layout :class="`${prefix}-layout`">
    ...
    >
      <t-tab-panel
        ...
        :key="routeItem.tabKey || `${routeItem.path}_${index}`"
        :value="routeItem.tabKey || routeItem.path"
...
import { businessTabKey, sameBusinessContext } from '@/utils/route/tabIdentity';
...
const activeRouteKey = computed(() => {
  const match = tabRouters.value.find(
    (item) => item.path === route.path && JSON.stringify(item.query || {}) === JSON.stringify(route.query || {}),
  );
  return match?.tabKey || match?.path || businessTabKey(route.path, route.query, userStore.businessContext);
});
const activeTabPath = ref<string | null>('');

const { locale } = useLocale();

const handleChangeCurrentTab = async (tabKey: string) => {
  const { tabRouters } = tabsRouterStore;
  const foundRoute = tabRouters.find((i) => (i.tabKey || i.path) === tabKey);
  if (!foundRoute) return;
  const currentHasContext = Object.keys(route.query).length > 0;
  const targetHasContext = Object.keys(foundRoute.query || {}).length > 0;
  // Persisted tabs from older builds did not retain business query context.
  // Ignore TDesign's initialization change instead of replacing a valid deep link.
  if (foundRoute.path === route.path && currentHasContext && !targetHasContext) return;
  if (foundRoute.businessContext && !sameBusinessContext(foundRoute.businessContext, userStore.businessContext)) {
    const target = router.resolve({ path: foundRoute.path, query: foundRoute.query }).fullPath;
    await userStore.switchBusinessContext(foundRoute.businessContext);
    window.location.assign(target);
    return;
  }
  router.push({ path: foundRoute.path, query: foundRoute.query });
};
...
const handleRemove = (options: TTabRemoveOptions) => {
  ...
  if ((options.value as string) === activeRouteKey.value)

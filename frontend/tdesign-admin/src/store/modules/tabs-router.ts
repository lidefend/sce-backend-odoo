...
import { businessTabKey } from '@/utils/route/tabIdentity';
...
export const useTabsRouterStore = defineStore('tabsRouter', {
  ...
  actions: {
    ...
    appendTabRouterList(newRoute: TRouterInfo) {
      ...
      const tabKey = newRoute.tabKey || businessTabKey(newRoute.path, newRoute.query);
      const homeIndex = this.tabRouters.findIndex((route: TRouterInfo) => route.isHome && route.path === newRoute.path);
      if (homeIndex >= 0) {
        this.tabRouterList = this.tabRouterList.map((route: TRouterInfo, index: number) =>
          index === homeIndex ? { ...route, ...newRoute, tabKey, isHome: true, isAlive: true } : route,
        );
        return;
      }
      if (!this.tabRouters.some((route: TRouterInfo) => (route.tabKey || route.path) === tabKey)) {
        this.tabRouterList = this.tabRouterList.concat({ ...newRoute, tabKey, isAlive: needAlive });
      }
    },
    // 处理关闭当前
    ...
    resetForSession() {
      this.tabRouterList = homeRoute.map((route) => ({ ...route, isAlive: true }));

...
assert.deepEqual(
  detailCollectionColumnPresentation(many2oneColumn, 7, true),
  { width: 260, align: 'left', ellipsis: true },
  'a late relation column must retain readable width and a supported readonly truncation affordance',
);
assert.deepEqual(
  detailCollectionColumnPresentation({ name: 'amount', label: '金额', ttype: 'monetary', required: false }, 3, true),
  { width: 140, align: 'right', ellipsis: false },
  'amount columns must keep their compact numeric presentation',
);
const mobileColumns = Array.from({ length: 8 }, (_, index) => ({
  name: `column_${index + 1}`,
  label: `第 ${index + 1} 列`,
  ttype: index === 5 ? 'monetary' : 'char',
  required: false,
}));
assert.deepEqual(
  detailCollectionMobileColumnSplit(mobileColumns),
  { primary: mobileColumns.slice(0, 6), additional: mobileColumns.slice(6) },
  'mobile detail cards preserve source order and keep trailing auxiliary facts behind an actionable disclosure',
);

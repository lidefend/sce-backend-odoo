<template>
  <TDesignCard
    v-bind="{ ...$attrs, ...semanticPrimitiveIdentity('ScCard') }"
    :title="title"
    :subtitle="subtitle"
    :bordered="bordered"
    :body-class-name="bodyClassName"
    :body-style="cardBodyStyle"
    :header-style="cardHeaderStyle"
    :data-appearance="appearance"
  >
    <template v-if="$slots.actions" #actions><slot name="actions" /></template>
    <slot />
  </TDesignCard>
</template>

<script setup lang="ts">
import { computed } from 'vue';
import { TDesignCard } from './tdesignPrimitiveBridge';
import { semanticPrimitiveIdentity } from './primitiveAdapter';

defineOptions({ inheritAttrs: false });
const props = withDefaults(defineProps<{
  title?: string;
  subtitle?: string;
  bordered?: boolean;
  bodyClassName?: string;
  appearance?: 'default' | 'summary' | 'task' | 'section' | 'task-section' | 'context' | 'relation' | 'form-section' | 'fact' | 'auth' | 'account' | 'main-surface' | 'flow' | 'kanban-record' | 'metric' | 'table' | 'record' | 'config';
}>(), {
  title: undefined,
  subtitle: undefined,
  bordered: true,
  bodyClassName: undefined,
  appearance: 'default',
});

type CardStyle = Record<string, string | number>;

const cardBodyStyle = computed<CardStyle>(() => {
  const styles: Partial<Record<NonNullable<typeof props.appearance>, CardStyle>> = {
    summary: { padding: '0' },
    task: { display: 'grid', alignContent: 'start', height: 'max-content', padding: 'var(--sc-space-xs) var(--sc-space-lg) var(--sc-space-md)' },
    'task-section': { display: 'grid', gridTemplateColumns: 'var(--sc-task-section-columns)', columnGap: 'var(--sc-space-xl)', rowGap: 'var(--sc-space-md)', padding: 'var(--sc-space-2xs) 0 var(--sc-space-lg)' },
    'form-section': { padding: '0' },
    fact: { padding: '0' },
    auth: { display: 'grid', gap: 'var(--sc-space-md)', padding: 'var(--sc-card-body-padding)' },
    account: { display: 'grid', gap: 'var(--sc-space-md)', padding: 'var(--sc-space-xl)' },
    'kanban-record': { display: 'grid', gap: 'var(--sc-card-gap)', padding: 'var(--sc-product-space-2)' },
    metric: { padding: 'var(--sc-space-xs)' },
    table: { padding: '0' },
    record: { display: 'grid', gap: 'var(--sc-product-space-3)', padding: 'var(--sc-product-space-3)' },
    config: { display: 'grid', gridTemplateRows: 'auto 1fr auto auto', gap: 'var(--sc-space-sm)', padding: 'var(--sc-space-sm)' },
    'main-surface': { padding: 'var(--sc-card-body-padding)' },
    flow: { padding: '0' },
  };
  return styles[props.appearance] || {};
});

const cardHeaderStyle = computed<CardStyle>(() => {
  if (props.appearance === 'task') return { alignItems: 'center', paddingBlock: 'var(--sc-space-sm) var(--sc-space-xs)' } as CardStyle;
  if (props.appearance === 'task-section') return { alignItems: 'center', padding: 'var(--sc-space-md) 0 var(--sc-space-xs)', borderTop: '1px solid var(--sc-app-border)' } as CardStyle;
  return {} as CardStyle;
});
</script>

<style scoped>
[data-appearance='summary'] { overflow: hidden; }
[data-appearance='fact'] { border: 0; background: transparent; box-shadow: none; }
[data-appearance='form-section'] { border: 0; border-radius: 0; background: transparent; box-shadow: none; }
[data-appearance='task-section'] { --sc-task-section-columns: repeat(2, minmax(0, 1fr)); border: 0; border-radius: 0; background: transparent; box-shadow: none; }
[data-appearance='auth'] { --sc-card-body-padding: var(--sc-space-xl); }
[data-appearance='main-surface'] { --sc-card-body-padding: 0 var(--sc-space-md) var(--sc-space-md); width: 100%; min-width: 0; }
[data-appearance='flow'] { width: 100%; min-width: 0; border: 0; background: transparent; box-shadow: none; }
@media (max-width: 640px) {
  [data-appearance='auth'] { --sc-card-body-padding: var(--sc-space-lg); }
  [data-appearance='main-surface'] { --sc-card-body-padding: 0 0 var(--sc-space-md); }
  [data-appearance='task-section'] { --sc-task-section-columns: minmax(0, 1fr); }
}
</style>

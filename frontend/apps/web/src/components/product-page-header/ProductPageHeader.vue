<template>
  <header
    ...
    :class="[`product-page-header--${model.variant}`, `product-page-header--${model.presentationMode}`]"
...
<style scoped>
.product-page-header { display:flex; align-items:center; justify-content:space-between; gap:var(--sc-space-sm); min-width:0; border:1px solid var(--sc-app-border); border-radius:var(--sc-product-panel-radius); background:var(--sc-app-panel); box-shadow:var(--sc-app-shadow); padding:var(--sc-space-xs) var(--sc-space-sm); }
.product-page-header__identity { display:grid; min-width:0; }
...
.product-page-header__status { flex:1 1 auto; min-width:0; margin-left:auto; text-align:right; }
.product-page-header__actions { display:flex; flex-wrap:wrap; align-items:center; justify-content:flex-end; gap:var(--sc-space-xs); }
.product-page-header--dialog { box-shadow:none; border-width:0 0 1px; border-radius:0; }
@media(max-width:860px){.product-page-header{align-items:stretch;flex-direction:column}.product-page-header__status{width:100%;margin-left:0;text-align:left}.product-page-header__actions{width:100%}.product-page-header h1{font-size:20px}}
</style>

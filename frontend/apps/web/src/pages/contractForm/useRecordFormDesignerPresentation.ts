...
export function useRecordFormDesignerPresentation(dependencies: PresentationDependencies) {
  const {
    ...
    nativeFormDesignFieldKeys,
    nativeFormDesignFieldLabels,
    ...
    selectedFormSettingsOrderTargetKey,
    v2ContractStore,
  } = dependencies;
  ...
  const activeContractModeFieldRows = computed<ContractFieldGovernanceRow[]>(() => {
    ...
    if (!isContractFieldOrderEditable.value || !fieldOrderDraft.value.length)
      return rowsWithDraftVisibility;
    const rank = new Map<string, number>(
      fieldOrderDraft.value.map((key: string, index: number) => [key, index]),
    );
    return rowsWithDraftVisibility.sort(
      (left, right) => (rank.get(left.fieldKey) ?? 9999) - (rank.get(right.fieldKey) ?? 9999),
    );
  });
  ...
  watch(
    [selectedFormSettingsFieldKey, selectedFormSettingsOrderTargetOptions],
    () => {
      ...
      if (!options.some((option) => option.fieldKey === selectedFormSettingsOrderTargetKey.value)) {
        const selectedIndex = currentFormOrderedFieldKeys.value.indexOf(
          selectedFormSettingsFieldKey.value,
        );
        const preferred =
          ...
          currentFormOrderedFieldKeys.value[selectedIndex - 1] ||
          options[0].fieldKey;
        selectedFormSettingsOrderTargetKey.value = options.some(
          (option) => option.fieldKey === preferred,
        )
          ? preferred
          : options[0].fieldKey;
      }
  ...
  function syncFieldOrderDraftWithDesignKeys(rawKeys = currentFormDesignFieldKeys.value) {
    ...
    if (!fieldOrderPreviewActive.value) {
      const same =
        keys.length === fieldOrderDraft.value.length &&
  ...
  const formVisibilityDraftFieldKeys = computed(() =>
    Array.from(
      new Set(
        [
  ...
  const hasFieldVisibilityChanges = computed(() =>
    formVisibilityDraftFieldKeys.value.some((fieldKey) => {
  ...
  const hasFieldGroupChanges = computed(() =>
    Object.keys(fieldGroupDraft).some((fieldKey) => {
      const draft = effectiveFieldGroupTitleForDraft(fieldKey);
      const base = normalizeFieldGroupTitle(
        fieldGroupSavedBase.value[fieldKey] || fieldGroupBase.value[fieldKey],
      );
  ...
  function effectiveGroupVisible(title: string) {
    ...
    if (!key) return true;
    if (Object.prototype.hasOwnProperty.call(groupVisibilityDraft, key))
      return groupVisibilityDraft[key] !== false;
    if (Object.prototype.hasOwnProperty.call(groupVisibilityBase.value, key))
      return groupVisibilityBase.value[key] !== false;
    return true;
  }
  ...
  function formConfigFieldLabelReplacementEntries() {
    return buildFormConfigFieldLabelReplacementEntries({
      ...
      resolveDescriptorLabel: (fieldKey) => {
        const descriptor = formFields.value[fieldKey] as Record<string, unknown> | undefined;
  ...
  function formDesignFieldLabel(fieldKey: string) {
    return resolveFormDesignFieldLabel({
      ...
      resolveDescriptorLabel: (key) => {
        const descriptor = formFields.value[key] as Record<string, unknown> | undefined;
        return String(descriptor?.string || descriptor?.label || '').trim();
      },
    });
  }
  ...
  const suggestedHiddenFieldRows = computed(() =>
    currentFormDesignFieldKeys.value

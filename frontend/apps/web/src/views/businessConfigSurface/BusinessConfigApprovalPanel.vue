    ...
    <aside class="approval-rule-panel" aria-label="审批规则设置">
      ...
      <div class="approval-config-grid">
        <ScCheckbox
          ...
          :model-value="form.approval_required"
          label="启用审批"
          @update:model-value="$emit('updateFormField', 'approval_required', $event); $emit('approvalRequiredChange')"
        >
          <span>启用审批</span>
        </ScCheckbox>
        <label>
    ...
    <section class="approval-steps" :class="{ 'approval-steps--disabled': !form.approval_required }">
      ...
      <div v-if="steps.length" class="approval-step-table" role="table" aria-label="审批步骤">
        ...
        >
          ...
          <ScTextField v-model="step.name" type="text" placeholder="例如：业务复核" :disabled="!form.approval_required" :label="`第${index + 1}步名称`" />
          </div>
          <div class="approval-step-cell">
            <ScSelect v-model="step.approval_scope_key" :disabled="!form.approval_required" :aria-label="`第${index + 1}步审批岗位`">
          ...
          <div class="approval-step-cell">
            <ScTextField v-model="step.amount_min" type="number" min="0" step="0.01" placeholder="不限制" :disabled="!form.approval_required" :label="`第${index + 1}步金额下限`" />
          </div>
          <div class="approval-step-cell">
            <ScTextField v-model="step.amount_max" type="number" min="0" step="0.01" placeholder="不限制" :disabled="!form.approval_required" :label="`第${index + 1}步金额上限`" />
          </div>

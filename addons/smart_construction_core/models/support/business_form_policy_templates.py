...
def _settlement_policy(title: str, counterparty_title: str) -> dict:
    ...
    return _policy(
        [
            ...
            _section("handling", "办理说明", ["settlement_description", "note", "attachment_ids"], 40),
...
def _payment_request_policy(title: str, counterparty_title: str) -> dict:
    ...
    return _policy(
        ...
        hide_create=("state", "name", "type", "receipt_type", "is_fully_paid"),
        readonly_only=(
...
BUSINESS_CATEGORY_FORM_POLICY_TEMPLATES = {
    ...
    "material.inbound": _policy(
        [
            ...
            _section("document_lines", "入库明细", ["line_ids"], 30),
            _section("handling", "办理说明", ["line_note_summary", "note", "attachment_ids"], 50),
            _section("execution", "执行结果", ["stock_picking_id", "source_transfer_outbound_id"], 70, collapsed=True, visible_profiles=READONLY_ONLY),
            _section("source_trace", "来源与系统追溯", list(FACT_TRACE_FIELDS), 90, collapsed=True, visible_profiles=READONLY_ONLY),
        ],
        required=("business_category_id", "project_id", "warehouse_id", "dest_location_id"),
        readonly_all=(
            "business_category_id",
            "state",
            "name",
    ...
    "material.outbound": _policy(
        [
            ...
            _section("document_lines", "出库明细", ["line_ids"], 30),
            _section("handling", "办理说明", ["note", "attachment_ids"], 50),
            _section(
                "execution",
                "执行结果",
                ["stock_picking_id", "transfer_inbound_id", "dest_warehouse_id", "dest_location_id"],
                70,
                collapsed=True,
                visible_profiles=READONLY_ONLY,
            ),
        ...
        required=("business_category_id", "project_id", "warehouse_id", "source_location_id"),
    ...
    "material.supplier_return": _policy(
        [
            _section("business_identity", "办理身份", ["state", "name"], 10),
            _section(
                "business_facts",
                "基本资料",
                [
            ...
            _section("document_lines", "退货明细", ["line_ids"], 30),
            _section("handling", "退货说明与附件", ["reason", "attachment_ids"], 50),
            _section("execution", "执行结果", ["stock_picking_id", "processing_advisory"], 70, collapsed=True, visible_profiles=READONLY_ONLY),
        ],
        required=("project_id", "warehouse_id", "source_location_id"),
        readonly_all=("state", "name", "amount_total", "stock_picking_id", "processing_advisory"),
        ledger=("stock_picking_id",),
    ),

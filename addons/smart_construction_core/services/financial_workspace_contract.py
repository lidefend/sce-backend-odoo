...
def build_financial_form_business_actions(env, model_name, record_id):
    ...
    if model == "sc.payment.execution":
        ...
        if not record:
            return None
        from .payment_execution_business_actions import (
            build_payment_execution_form_actions,
            build_payment_execution_task_semantics,
        )
        semantics = build_payment_execution_task_semantics(record)
        actions = build_payment_execution_form_actions(record, task_semantics=semantics)
        return {"actions": actions, "task_semantics": semantics} if actions or semantics else None
        ...
        for method in filter(None, methods):
            ...
            actions.append({
                ...
                "mutation": {"type": "record_action", "model": "payment.request", "operation": action_key,
                             ...
                             "params": {"id": "$record_id", "action": "$operation",

...
def migrate(cr, version):
    ...
    if row_count:
        raise RuntimeError(
            "SETTLEMENT_ORDER_P2_HISTORY_NOT_EXTRACTED: table=%s rows=%s"
            % (TABLE, row_count)
        )

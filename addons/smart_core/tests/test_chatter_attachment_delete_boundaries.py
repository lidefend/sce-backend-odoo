...
def _load_handler():
    ...
    module_name = "odoo.addons.smart_core.handlers.chatter_attachment_delete"
    sys.modules.pop(module_name, None)
    spec = importlib.util.spec_from_file_location(module_name, root / "handlers" / "chatter_attachment_delete.py")
...
class TestChatterAttachmentDeleteBoundaries(unittest.TestCase):
    def test_missing_target_fails_before_environment_access(self):
        module = _load_handler()
        result = module.ChatterAttachmentDeleteHandler(env={}, params={}).handle()
        self.assertFalse(result["ok"])
        self.assertEqual(result["code"], 400)

    def test_invalid_identifiers_fail_closed(self):
        module = _load_handler()
        result = module.ChatterAttachmentDeleteHandler(
            env={"x.model": object()},
            params={"model": "x.model", "res_id": "bad", "attachment_id": "also-bad"},
        ).handle()
        self.assertFalse(result["ok"])
        self.assertEqual(result["code"], 400)
    ...
    def test_source_authority_is_an_explicit_write_proxy(self):
        module = _load_handler()
        contract = module.ChatterAttachmentDeleteHandler.source_authority_contract()
        self.assertFalse(contract["projection_only"])
        self.assertTrue(contract["write_proxy"])
        self.assertIn("ir.attachment", contract["authorities"])
    ...
    def test_timeline_projects_delete_only_with_exact_intent(self):
        ...
        self.assertIn('"delete_intent": "chatter.attachment.delete" if can_delete else ""', timeline)
        self.assertIn("is_direct_attachment", timeline)

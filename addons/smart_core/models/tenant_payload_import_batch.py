...
def _assert_signed_import_boundary(env):
    ...
    # TransactionCase exercises the model boundary without a maintenance
    # process. Production configuration forbids Odoo test mode.
    ...
    if (
        len(expected_capability) != 64
        or not hashlib.compare_digest(actual_capability, expected_capability)
    ):
